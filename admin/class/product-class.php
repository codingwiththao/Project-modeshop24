...
class product {
    ...
    public function insert_product() {
        ...
        $filetarget = basename($_FILES['product-img']['name']);
        ...
        if(file_exists("uploads/$filetarget")) {
            ...
            if($filetype != "jpg" && $filetype != "png" && $filetype != "jpeg") {
                $alert = "Nur .jpg-, .png-, .jpeg-File wählen ";
                return $alert;
            }
            else {
                if ($filesize > 1000000) {
                    $alert = "File darf nicht mehr als 1MB überschritten";
                    return $alert;
                }
                else {
                    move_uploaded_file($_FILES['product-img']['tmp_name'],"uploads/".$_FILES['product-img']['name']);
                    ...
                    if($result) {
                        ...
                        $filename = $_FILES['product-img-desc']['name'];
                        $filetmp = $_FILES['product-img-desc']['tmp_name'];
    ...
    public function update_brand($cartegory_id, $brand_name, $brand_id) {
        ...
        SET brand_name = '$brand_name', cartegory_id = '$cartegory_id' 
        WHERE brand_id = '$brand_id' ";
        $result = $this->db->update($query);
        // Nachdem diese erneute Verarbeitung abgeschlossen ist, wird automatisch auf diese Seite umgeschaltet
        header('location:brand-list.php'); 
        return $result;
    }
